...
// 删除: RKNPU1 兼容 (process_u8)、FP32 兼容 (process_fp32)、RV1106/1103 专用分支
// 仅保留: process_i8 (RK3568 RKNPU2 INT8 量化模型输出的标准解析函数)
...
/* ---- RK3568 只需要 INT8 (affine symmetric, int8) 的量化/反量化 ---- */
...
static void compute_dfl(float *tensor, int dfl_len, float *box)
{
    for (int b = 0; b < 4; b++)
    {
        float exp_t[16]; // dfl_len 最大值一般是 16 (YOLOv8)，栈上足够
        float exp_sum = 0;
        float acc_sum = 0;
...
/* ---- 唯一保留的 process: RK3568/RKNPU2 INT8 (i8) ---- */
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            if (max_score > score_thres_i8)
            {
                offset = i * grid_w + j;
                float box[4];
                float before_dfl[64]; // dfl_len*4, dfl=16 时 64 够了
                 ...
                 object_detect_result_list *od_results)
{
    ...
    // RK3568: output_attrs[0].dims 顺序是 [1, dfl_len*4, 80, 80] -> dims[1]/4 = dfl_len
